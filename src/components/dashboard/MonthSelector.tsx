...
export function MonthSelector({ selectedMonth, onMonthChange, variant = "default" }: MonthSelectorProps) {
  ...
  const btnClass = variant === "overlay"
    ...
    ? "h-4 w-4 md:h-5 md:w-5 rounded-full text-white/80 md:text-foreground hover:text-white md:hover:text-foreground hover:bg-white/15 md:hover:bg-muted/60"
  ...
  const labelClass = variant === "overlay"
    ...
    : variant === "overlay-mobile"
    ? "text-[9px] md:text-[10px] font-semibold text-white md:text-foreground min-w-[72px] md:min-w-[100px] text-center select-none"
  ...
  return (
    <div className="flex items-center gap-0">
      <Button variant="ghost" size="icon" onClick={handlePrev} className={btnClass}>
        <ChevronLeft className="h-2 w-2 md:h-2.5 md:w-2.5" />
      </Button>
