...
export default function RegistrationFlow({ onBack, onGoogleSignUp }: RegistrationFlowProps) {
  ...
  // Email verification state (IBBRA flow)
  ...
  const stepDescriptions: Record<Step, string> = {
    ...
    email_verification: "Confirme seu e-mail para continuar.",
  ...
  const handleClientAnswer = (answer: boolean) => {
    ...
    setStep(answer ? "cpf_validation" : "signup_method");
  };
  ...
  const handleValidateCpf = async () => {
    ...
    try {
      ...
      if (result.found) {
        ...
        setValidationError("Cliente não encontrado na base IBBRA. Verifique o CPF ou entre em contato.");
      }
    } catch {
  ...
  const handleVerifyEmail = async () => {
    ...
    try {
      ...
      if (result.match && result.email) {
        ...
        setEmailVerifyError("O e-mail informado não corresponde ao cadastro IBBRA. Tente novamente.");
      }
    } catch {
    ...
    <div className="space-y-6">
      ...
      <AnimatePresence mode="wait">
        ...
        {step === "client_question" && (
          <motion.div key="client_question" variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.25 }} className="space-y-5">
            <div className="space-y-2">
              <Label className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Você já é cliente IBBRA?</Label>
              <p className="text-xs text-muted-foreground/70">Clientes IBBRA possuem validação facilitada.</p>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <button onClick={() => handleClientAnswer(true)} className={`group relative flex flex-col items-center gap-3 p-6 rounded-xl border-2 transition-all duration-200 hover:shadow-md ${isIbbraClient === true ? "border-primary bg-primary/5" : "border-border hover:border-primary/40"}`}>
                <UserCheck className="h-8 w-8 text-primary/70 group-hover:text-primary transition-colors" />
                <span className="text-sm font-medium">Sim</span>
              </button>
              <button onClick={() => handleClientAnswer(false)} className={`group relative flex flex-col items-center gap-3 p-6 rounded-xl border-2 transition-all duration-200 hover:shadow-md ${isIbbraClient === false ? "border-primary bg-primary/5" : "border-border hover:border-primary/40"}`}>
        ...
        {/* STEP: CPF Validation */}
        {step === "cpf_validation" && (
          <motion.div key="cpf_validation" variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.25 }} className="space-y-5">
            <div className="space-y-4">
              <div>
                <Label className="text-xs font-medium uppercase tracking-wider text-muted-foreground">CPF do cliente IBBRA</Label>
                <Input value={cpf} onChange={(e) => handleCpfChange(e.target.value)} placeholder="000.000.000-00" className="h-12 mt-2 text-sm input-executive" inputMode="numeric" maxLength={14} />
              </div>
              {validationError && (
                <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="flex items-start gap-3 p-4 rounded-lg bg-destructive/5 border border-destructive/20">
              ...
              {cpfDuplicateEmail && (
                <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="flex items-start gap-3 p-4 rounded-lg bg-destructive/5 border border-destructive/20">
                  ...
                  <div className="text-sm">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">E-mail vinculado: <span className="font-mono">{maskEmail(cpfDuplicateEmail)}</span></p>
                    <button onClick={onBack} className="text-primary underline mt-1 text-xs hover:text-primary/80">Recuperar senha</button>
                  </div>
                </motion.div>
              )}
            </div>
            <div className="flex gap-3">
              <Button variant="outline" className="h-12" onClick={() => { setStep("client_question"); setCpf(""); setValidationResult(null); setValidationError(""); }}>
                <ArrowLeft className="h-4 w-4" />
              </Button>
              <Button className="flex-1 h-12 text-sm font-semibold" onClick={handleValidateCpf} disabled={isLoading || cpf.replace(/\D/g, "").length !== 11}>
        ...
        {/* STEP: Email Verification (IBBRA) */}
        {step === "email_verification" && (
          <motion.div key="email_verification" variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.25 }} className="space-y-5">
            ...
            <div>
              <Label className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Confirme seu e-mail</Label>
              <Input
                type="email"
            ...
            <div className="flex gap-3">
              ...
              <Button className="flex-1 h-12 text-sm font-semibold" onClick={handleVerifyEmail} disabled={isLoading || !ibbraEmailInput.trim()}>
        ...
        {/* STEP: Signup Method */}
        {step === "signup_method" && (
          <motion.div key="signup_method" variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.25 }} className="space-y-5">
            <Label className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Como deseja criar sua conta?</Label>

            {onGoogleSignUp && (
              <Button type="button" variant="outline" className="w-full h-12 text-sm" onClick={handleChooseGoogle}>
            ...
            <Button className="w-full h-12 text-sm font-semibold" onClick={handleChooseManual}>
              Cadastrar com email e senha
              <ArrowRight className="h-4 w-4 ml-2" />
            </Button>

            <Button variant="ghost" className="w-full h-10 text-sm text-muted-foreground" onClick={() => { if (isIbbraClient) setStep("email_verification"); else setStep("client_question"); }}>
        ...
        {step === "form_standard" && (
          <motion.div key="form_standard" variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.25 }} className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <FieldGroup label="Data de nascimento">
                <Input value={birthDate} onChange={(e) => setBirthDate(formatBirthDate(e.target.value))} placeholder="DD/MM/AAAA" className="h-11 text-sm input-executive" inputMode="numeric" maxLength={10} />
              </FieldGroup>
              <FieldGroup label="CPF">
                <Input value={cpf} onChange={(e) => handleCpfChange(e.target.value)} onBlur={handleCpfBlur} placeholder="000.000.000-00" className="h-11 text-sm input-executive" inputMode="numeric" maxLength={14} />
              </FieldGroup>
            </div>

            {cpfDuplicateEmail && (
              <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="flex items-start gap-3 p-3 rounded-lg bg-destructive/5 border border-destructive/20">
                <Mail className="h-4 w-4 text-destructive shrink-0 mt-0.5" />
                <div className="text-xs">
                  <p className="font-medium text-destructive">CPF já cadastrado</p>
                  <p className="text-muted-foreground mt-0.5">E-mail vinculado: <span className="font-mono">{maskEmail(cpfDuplicateEmail)}</span></p>
                  <button onClick={onBack} className="text-primary underline mt-1 hover:text-primary/80">Recuperar senha</button>
                </div>
              </motion.div>
            )}
            ...
            <div className="grid grid-cols-2 gap-3">
              <FieldGroup label="Telefone">
                <Input value={phone} onChange={(e) => setPhone(formatPhone(e.target.value))} placeholder="(11) 99999-0000" className="h-11 text-sm input-executive" inputMode="tel" maxLength={15} />
              </FieldGroup>
            ...
            <div className="flex items-start gap-2 pt-2">
              <Checkbox id="consent" checked={consentAccepted} onCheckedChange={(v) => setConsentAccepted(v === true)} className="mt-0.5" />
              <label htmlFor="consent" className="text-xs text-muted-foreground leading-relaxed cursor-pointer">
                Aceito os <a href="/termos-de-uso" target="_blank" className="text-primary underline">Termos de Uso</a>,
                a <a href="/politica-de-privacidade" target="_blank" className="text-primary underline">Política de Privacidade</a> e
            ...
            <div className="flex gap-3 pt-1">
              ...
              <Button className="flex-1 h-12 text-sm font-semibold" onClick={() => setStep("family_question")} disabled={!email.trim() || !password || password.length < 6 || password !== confirmPassword || !fullName.trim() || !consentAccepted || !!cpfDuplicateEmail}>
                Continuar
                <ArrowRight className="h-4 w-4 ml-2" />
              </Button>
            </div>
        ...
        {/* STEP: IBBRA Client Form (pre-filled, email from verification) */}
        {step === "form_ibbra" && (
          <motion.div key="form_ibbra" variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.25 }} className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <FieldGroup label="Email (verificado)">
                <Input value={ibbraVerifiedEmail || email} readOnly className="h-11 text-sm input-executive bg-muted/50 cursor-not-allowed" />
              </FieldGroup>
            </div>
            ...
            <div className="flex items-start gap-2 pt-2">
              <Checkbox id="consent-ibbra" checked={consentAccepted} onCheckedChange={(v) => setConsentAccepted(v === true)} className="mt-0.5" />
            ...
            <div className="flex gap-3 pt-1">
              ...
              <Button className="flex-1 h-12 text-sm font-semibold" onClick={() => setStep("family_question")} disabled={!password || password.length < 6 || password !== confirmPassword || !consentAccepted}>
                Continuar
                <ArrowRight className="h-4 w-4 ml-2" />
              </Button>
            </div>
        ...
        {step === "family_question" && (
          <motion.div key="family_question" variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.25 }} className="space-y-5">
            <div className="space-y-2">
              <Label className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Cadastrar familiares?</Label>
              <p className="text-xs text-muted-foreground/70">Você pode adicionar parentes agora ou depois.</p>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <button onClick={() => setStep("family_form")} className="group relative flex flex-col items-center gap-3 p-6 rounded-xl border-2 border-border hover:border-primary/40 transition-all duration-200 hover:shadow-md">
              ...
              <button onClick={handleSubmit} disabled={isLoading} className="group relative flex flex-col items-center gap-3 p-6 rounded-xl border-2 border-border hover:border-primary/40 transition-all duration-200 hover:shadow-md">
                {isLoading ? <Loader2 className="h-8 w-8 animate-spin text-primary/70" /> : <Check className="h-8 w-8 text-primary/70 group-hover:text-primary transition-colors" />}
                <span className="text-sm font-medium">{isLoading ? "Criando..." : "Não, finalizar"}</span>
              </button>
            </div>
            <Button variant="ghost" className="w-full h-10 text-sm text-muted-foreground" onClick={() => { if (isIbbraClient && validationResult?.found) setStep("form_ibbra"); else setStep("form_standard"); }}>
              <ArrowLeft className="h-4 w-4 mr-2" />
        ...
        {/* STEP: Family Form */}
        {step === "family_form" && (
          <motion.div key="family_form" variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.25 }} className="space-y-4">
            {familyMembers.map((member, index) => (
              <div key={index} className="space-y-3 p-4 rounded-lg border border-border bg-muted/20">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Familiar {index + 1}</span>
                  {familyMembers.length > 1 && (
                    <button onClick={() => removeFamilyMember(index)} className="text-destructive/70 hover:text-destructive transition-colors">
                ...
                <FieldGroup label="Grau de parentesco">
                  <Select value={member.relationship} onValueChange={(v) => updateFamilyMember(index, "relationship", v)}>
                    <SelectTrigger className="h-11 text-sm input-executive"><SelectValue placeholder="Selecione..." /></SelectTrigger>
                    <SelectContent>{RELATIONSHIP_OPTIONS.map((opt) => (<SelectItem key={opt} value={opt}>{opt}</SelectItem>))}</SelectContent>
                  </Select>
                </FieldGroup>
                <FieldGroup label="Nome completo">
                ...
                <div className="grid grid-cols-3 gap-3">
                  <FieldGroup label="Idade"><Input value={member.age} onChange={(e) => updateFamilyMember(index, "age", e.target.value)} placeholder="Ex: 35" className="h-11 text-sm input-executive" inputMode="numeric" /></FieldGroup>
                  <FieldGroup label="Telefone"><Input value={member.phone} onChange={(e) => updateFamilyMember(index, "phone", formatPhone(e.target.value))} placeholder="(11) 99999-0000" className="h-11 text-sm input-executive" inputMode="tel" maxLength={15} /></FieldGroup>
                  <FieldGroup label="Email"><Input type="email" value={member.email} onChange={(e) => updateFamilyMember(index, "email", e.target.value)} placeholder="email@email.com" className="h-11 text-sm input-executive" /></FieldGroup>
                </div>
              </div>
            ))}
