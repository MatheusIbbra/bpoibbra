...
export default function Despesas() {
  ...
  // Only show classified expense transactions
  ...
  const classifiedTransactions = transactions?.filter(
    t => t.category_id !== null && !t.is_ignored && t.type === "expense"
  ) || [];

  const deleteTransaction = useDeleteTransaction();

  const totalExpenses = classifiedTransactions.reduce((acc, t) => acc + Number(t.amount), 0);
    ...
    return (
      <AppLayout title="Despesas">
        <div className="space-y-6">
          <BaseRequiredAlert action="visualizar despesas" />
          <Card>
            <CardContent className="flex flex-col items-center justify-center py-12 text-center">
              <TrendingDown className="h-12 w-12 text-muted-foreground mb-4" />
              <h3 className="text-lg font-semibold">Selecione uma base</h3>
              <p className="text-muted-foreground">
                Selecione uma base específica no menu superior para visualizar as despesas.
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-lg">
              {isLoading ? "Carregando..." : `${classifiedTransactions.length} despesas classificadas`}
            </CardTitle>
          </CardHeader>
          <CardContent>
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-8 text-center">
                ...
                <h3 className="text-lg font-semibold">Nenhuma despesa classificada</h3>
                <p className="text-muted-foreground">
                  As despesas aparecerão aqui após serem classificadas.
                </p>
              </div>
            ) : (
              <div className="space-y-2">
                {classifiedTransactions.map((transaction) => (
                  ...
                  >
                    <div className="hidden sm:flex h-10 w-10 items-center justify-center rounded-full bg-destructive/10 text-destructive shrink-0">
