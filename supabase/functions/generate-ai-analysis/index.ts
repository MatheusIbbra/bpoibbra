  ...
  try {
    ...
    for (let attempt = 0; attempt <= 1; attempt++) {
      try {
        ...
        textContent = aiData.candidates?.[0]?.content?.parts?.[0]?.text || "";
        tokenUsage = aiData.usageMetadata?.totalTokenCount || 0;
        break;
      } catch (err) {
        console.error(`[generate-ai-analysis] Gemini call failed (attempt ${attempt}):`, err);
        if (attempt < 1) continue;
        return new Response(
          JSON.stringify({ error: "AI service timeout" }),
    ...
    if (organization_id) {
      ...
      await adminClient.from("api_usage_logs").insert({
        ...
        request_metadata: { model: "gemini-2.5-flash" },
      });
    }
