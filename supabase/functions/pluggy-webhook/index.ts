            ...
            if (newTxIds.length > 0) {
              ...
              for (const tx of newTxIds) {
                ...
                try {
                  ...
                  // AI fallback via Gemini 2.5 Flash
                  if (!classified && GEMINI_API_KEY) {
                    ...
                    const fullPrompt = `Classifique transações. APENAS JSON. Categorias (${tx.type}):\n${categoryList}\nFormato: {"category_id":"uuid|null","confidence":0-1}\n\n"${tx.description}" R$${tx.amount.toFixed(2)} ${tx.type === "income" ? "Receita" : "Despesa"}`;
                    ...
                    try {
                      ...
                      console.warn(`[WEBHOOK-CLASSIFY] Gemini error for TX ${tx.id}:`, aiErr);
                    }
                  }
                } catch (classErr) {
