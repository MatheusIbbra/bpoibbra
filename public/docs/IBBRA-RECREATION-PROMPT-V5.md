# IBBRA — Prompt de Recriação v5.0
...
-- BANK_CONNECTIONS_SAFE (view - oculta tokens)
CREATE VIEW public.bank_connections_safe AS
  SELECT id, organization_id, user_id, provider, provider_name,
    external_consent_id, external_account_id, status, last_sync_at,
    sync_error, token_expires_at, metadata, created_at, updated_at,
    NULL::text as access_token_encrypted, NULL::text as refresh_token_encrypted
...
CREATE TABLE public.open_finance_accounts (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  item_id UUID NOT NULL REFERENCES open_finance_items(id),
  organization_id UUID NOT NULL REFERENCES organizations(id),
...
-- OPEN_FINANCE_RAW_DATA, OPEN_FINANCE_SYNC_LOGS, SYNC_AUDIT_LOGS
...
-- (Ver documentação V6 para definições completas)
...
*Prompt de recriação v5.0 — 16 de Fevereiro de 2026*