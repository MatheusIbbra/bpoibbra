# IBBRA — Documentação Técnica Completa v7.0
...
### 3.5 Recuperar Acesso
- Tela de "Esqueci minha senha" usa input nativo `<input>` (não react-hook-form) para garantir compatibilidade mobile
- Envia link de recuperação via `supabase.auth.resetPasswordForEmail`
...
| `delete-client` | Remove cliente/organização do sistema |
...
| `useCashFlowReport` | Relatório de fluxo de caixa (filtra apenas contas corrente, poupança, investimento, espécie) |
...
- **Saldos de cartão excluídos do Saldo Total Acumulado** (são passivos, não ativos)

### 14.5 Tratamento de Transações de Cartão
- Transações de cartão classificadas como `expense` por padrão
- Pagamentos de fatura detectados por padrões estritos (ex: "pagamento de fatura", "pagamento recebido") → reclassificados como `transfer` com `is_ignored = true`
- Regime de Caixa: ignora compras individuais, contabiliza apenas pagamento da fatura
...
- **DRE**: Demonstração do Resultado do Exercício (fixo em regime de Competência)
- **Fluxo de Caixa**: Entradas e saídas por período (exclui cartões de crédito — considera apenas contas corrente, poupança, investimento e espécie)
...
### Regimes Contábeis
- **Regime de Caixa**: utiliza `payment_date` (ou `date`). Ignora compras de cartão, contabiliza pagamento de fatura
- **Regime de Competência**: utiliza `accrual_date` (com fallback para `date`). Reconhece cada compra na data de ocorrência
...
### Fluxo de Caixa — Filtro de Contas
- **Tipos permitidos:** `checking`, `savings`, `investment`, `cash`
- **Cartões de crédito (`credit_card`) completamente excluídos** do relatório de Fluxo de Caixa
- Filtragem aplicada no nível do banco de dados (query por `account_id`)
- `getLegacyInitialBalanceAdjustment` também recebe `allowedAccountIds` para não incluir saldos de cartão
...
*Documento gerado automaticamente — v7.0 — 16 de Fevereiro de 2026*
